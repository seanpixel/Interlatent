...
TranscoderPipeline(db, "llm.layer.-1", k=8, epochs=3).run()
SAEPipeline(db, "llm.layer.-1", k=8, epochs=3).run()
```
Both pipelines backfill new activations:
- `latent:llm.layer.-1` (transcoder latents)
- `latent_sae:llm.layer.-1` (SAE latents)
...
python -m interlatent.vis.plot latents_llm.db --layer latent:llm.layer.-1 --channel 0 --prompt-index 1
```
...
- **Visualization quickies**:
  ...
  - Plot: `python -m interlatent.vis.plot latents.db --layer latent:llm.layer.-1 --channel 0 --prompt-index 0`
...
```python
train_linear_probe(db, layer="llm.layer.-1", target_key="label", task="classification")
TranscoderPipeline(db, "llm.layer.-1", k=8).run()
SAEPipeline(db, "llm.layer.-1", k=8).run()
```

**Search & plot:**
```bash
python -m interlatent.vis.search latents.db --layer-prefix latent: --token-like bomb --top 10
python -m interlatent.vis.plot latents.db --layer latent:llm.layer.-1 --channel 0 --prompt-index 0
```
